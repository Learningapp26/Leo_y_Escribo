...
const totalExercises = blSyllableJoin.length + blWordJoin.length

function ActividadBlFinalPage() {
  ...
  const joinItem = blSyllableJoin[joinIndex]
  const isLastJoin = joinIndex === blSyllableJoin.length - 1
  ...
  const checkJoin = () => {
    ...
    registrarProgreso({
      actividad: 'bl-final',
      correcto: isCorrect,
      detalle: { leccionId: 'bl', fase: 'silaba', ejercicioId: joinItem.id },
    })
  }
  ...
  const sentenceItem = blWordJoin[sentenceIndex]
  const isLastSentence = sentenceIndex === blWordJoin.length - 1
  ...
  useEffect(() => {
    if (finished) registrarLeccionCompletada('bl')
  }, [finished])
    ...
    return (
      <main className={`page ${themeClass}`}>
        <Card className="selection-card">
          ...
          <h1>¡Terminaste la lección de la combinación BL!</h1>

          <p className="text-instruction">
            Aprendiste el sonido /bl/, sus sílabas bla, ble, bli, blo, blu y
            palabras nuevas.
          </p>
          ...
          <Button
            ...
            fullWidth
            onClick={() =>
              playAudio('/audio/lecciones/bl/felicitacion-final.mp3')
            }
          >
  ...
  return (
    <main className={`page ${themeClass}`} aria-labelledby="bl-final-title">
      <BackButton label="Volver a la lección" to="/lecciones/bl" />

      <header className="text-center">
        <span className="text-ui-label">Actividad final</span>

        <h1 id="bl-final-title">Practica todo lo aprendido</h1>
      </header>
        ...
        <Card className="selection-card">
          <div className="selection-instructions">
            ...
            <Button
              variant="audio"
              icon={Volume2}
              onClick={() => playAudio(blJoinInstructionAudio)}
            >
        ...
        <Card className="selection-card">
          <div className="selection-instructions">
            ...
            <Button
              variant="audio"
              icon={Volume2}
              onClick={() => playAudio(blSentenceFormation.instructionAudio)}
            >
              Escuchar instrucción
            </Button>
          </div>
...
export default ActividadBlFinalPage