...
function ActividadJSilabasPage() {
  const [phaseIndex, setPhaseIndex] = useState(0)

  const [countIndex, setCountIndex] = useState(0)
  const [countSelected, setCountSelected] =
    useState(null)
  ...
  const [selectedVowels, setSelectedVowels] =
    useState([])
  ...
  const countWord =
    jSyllableCountWords[countIndex]

  const assocItem =
    jSyllableAssociation[assocIndex]

  const checkCount = () => {
    if (countSelected === null) return

    const isCorrect =
      countSelected === countWord.syllables.length
  ...
  const checkVowels = () => {
    ...
    const isCorrect =
      selectedVowels.length ===
        targetVowels.length &&
      targetVowels.every((vowel) =>
  ...
  return (
    <main
      ...
      aria-labelledby="j-silabas-title"
    >
      <BackButton
      ...
      {phase === 'contar' && (
        <Card className="completion-card">
          ...
          <Button
            ...
            icon={Volume2}
            onClick={() =>
              playIfAvailable(countWord.audio)
            }
          >
            Escuchar {countWord.name}
          </Button>

          <div className="completion-bank">
            {countWord.syllables.map(
              (syllable, index) => (
                <span
          ...
          {countResult === 'correct' && (
            <p
              ...
              role="status"
            >
              ¡Correcto! {countWord.name} tiene{' '}
              {countWord.syllables.length} sílabas.
            </p>
          )}
          ...
          {countResult === 'retry' && (
            <p
              ...
              role="status"
            >
              Escucha otra vez y cuenta con cuidado.
            </p>
          )}
      ...
      {phase === 'buscar' && (
        <Card className="syllables-card">
          <p className="text-instruction">
            Encuentra y toca todas las sílabas que
            comienzan con la letra J.
          </p>

          <div className="syllables-options">
            {jSyllableSearch.options.map(
              (option) => {
                const selected =
                  searchSelected.includes(option.id)
          ...
          {searchResult === 'retry' && (
            <p
              ...
              role="status"
            >
              Revisa nuevamente y toca solamente
              las sílabas que comienzan con J.
            </p>
          )}
      ...
      {phase === 'letra' && (
        <Card className="syllables-card">
          <p className="text-instruction">
            Toca cada sílaba con J y escucha cómo
            suena.
          </p>
          ...
          <div
            ...
            aria-label="Letra J"
          >
            <span className="text-letter">J</span>
            <span className="text-letter">j</span>
          </div>
          ...
          <Button
            ...
            onClick={() =>
              playIfAvailable(
                jLetterPresentation.soundAudio,
              )
            }
          >
            Escuchar el sonido de la J
          </Button>

          <div className="syllables-options">
            {jLetterPresentation.combinations.map(
              (item) => {
                const vowel =
                  item.syllable.slice(1)
          ...
          {vowelResult === 'correct' && (
            <p
              ...
              role="status"
            >
              ¡Muy bien! Encontraste ja, je, ji, jo
              y ju.
            </p>
          )}
          ...
          {vowelResult === 'retry' && (
            <p
              ...
              role="status"
            >
              Escucha nuevamente y toca todas las
              sílabas con J.
            </p>
          )}
      ...
      {phase === 'asociar' && (
        <Card className="selection-card">
          <p className="text-instruction">
            Observa la imagen, lee la palabra y
            escucha la sílaba con J.
          </p>
          ...
          <p
            className="text-word"
            aria-label={assocItem.word}
          >
            <span className="text-syllable">
              {assocItem.highlighted}
            </span>
          ...
          <div className="selection-options">
            <Button
              ...
              onClick={() =>
                playIfAvailable(
                  assocItem.syllableAudio,
                )
              }
            >
              Escuchar {assocItem.highlighted}
            </Button>
...
export default ActividadJSilabasPage