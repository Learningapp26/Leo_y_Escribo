...
import {
  jImagePool,
  jPairInstructionAudio,
  jSelectionInstructionAudio,
  jSoundIntro,
  jSoundPairMatching,
  jSoundSelectionAnswerIds,
  jSyllableHearing,
  jSyllableHearingInstructionAudio,
...
function ActividadJImagenesPage() {
  ...
  const syllableItem = jSyllableHearing[syllableIndex]

  const isPairsFinished =
    matchedIds.length === jSoundPairMatching.length
  ...
  const checkSelection = () => {
    const selectedIncorrectWord = selectedIds.some(
      (id) => !jSoundSelectionAnswerIds.includes(id),
    )

    const missingCorrectWord =
      jSoundSelectionAnswerIds.some(
    ...
    if (
      ...
      setSelectionError(
        'Revisa nuevamente: seleccionaste una o más palabras que no comienzan con /j/ y todavía faltan palabras correctas.',
      )
      return
    }
    ...
    if (selectedIncorrectWord) {
      setSelectionError(
        'Una de las palabras seleccionadas no comienza con el sonido /j/.',
      )
      return
    }

    setSelectionError(
      '¡Casi lo logras! Todavía falta seleccionar una o más palabras que comienzan con el sonido /j/.',
    )
  }
      ...
      {phase === 'sonido' && (
        <Card className="selection-card">
          <p className="text-instruction">
            Escucha el sonido de la letra j y la
            palabra dedo.
          </p>
          ...
          <img
            ...
            src={jSoundIntro.mainWord.image}
            alt={jSoundIntro.mainWord.name}
          />

          <span className="text-word">
            {jSoundIntro.mainWord.name}
          </span>
          ...
          <Button
            ...
            onClick={() =>
              playIfAvailable(
                jSoundIntro.letterAudio,
              )
            }
          >
            Escuchar el sonido /j/
          </Button>
      ...
      {phase === 'parejas' && (
        <Card className="selection-card">
          ...
          <Button
            ...
            icon={Volume2}
            onClick={() =>
              playIfAvailable(jPairInstructionAudio)
            }
          >
            Escuchar instrucción
          </Button>
...
export default ActividadJImagenesPage